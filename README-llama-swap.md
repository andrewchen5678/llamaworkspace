# Single-machine: Gemma 4 E4B + MTP speculative decoding (llama-swap)

> One of two workflows in this repo — see [`README.md`](./README.md) for the
> overview and shared setup. This is the **single-machine** path. It shares port
> `8090` with the [cluster workflow](./README-cluster.md); **run only one at a time.**
...
those tokens faster. [`llama-swap`](https://github.com/mostlygeek/llama-swap)
fronts it for on-demand load and idle auto-unload.
...
**Prerequisite:** the CI-built `llama-server` in `./llama.cpp/bin` (run
`./fetch-llamacpp-rpc.sh`). See [Requirements in `README.md`](./README.md#requirements).
...
## 4. Auto-unload when idle (llama-swap)
...
**Install** (the llama-swap *proxy* — separate from `llama-server`; no Homebrew
formula, so drop the prebuilt binary on your `PATH`):
...
The config (`llama-swap.yaml`) defines three models on the same weights, with a
5-minute idle `ttl` — all invoking `./llama.cpp/bin/llama-server`:
